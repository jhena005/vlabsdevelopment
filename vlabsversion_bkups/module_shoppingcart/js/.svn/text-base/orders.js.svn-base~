...
var ord_table;
var orditems_table;
var ordersphpURL = "../shoppingcart/server/orders.php";
...
function ord_reload()
{
    ...
    $.ajax({
        ...
        success: function(data){
        	...
        	$('#ordersContainer').html( '<table cellpadding="0" cellspacing="0" border="0" class="display" id="ordersTable"></table>' );
...
{	
	...
	if(role=="admin" && aData[5] == "PENDING APPROVAL")
	{
		sOut += '	<button id="approveOrder'+aData[0]+'">Approve</button>';
		sOut += '	<button id="declineOrder'+aData[0]+'">Decline</button>';	
	}
	if(role=="admin" && (aData[5] == "APPROVED" || aData[5] == "CHARGED" || aData[5] == "DELIVERED"))
	{
