...
class GoogleRequest {
    ...
    function SendNotificationRequest($serial) {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
					  <notification-history-request xmlns=\"" . $this->schema_url . "\">";
        $postargs .= "<serial-number>" . $serial . "</serial-number>";
        $postargs .= "</notification-history-request>";
        return $this->SendReq($this->report_url, $this->GetAuthenticationHeaders(), $postargs);
    }
